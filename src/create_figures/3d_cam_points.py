...
def ransac_ray_intersection(
    ...
    origins          : (N, 3)
    directions       : (N, 3)  — unit vectors
    weights          : (N,)    — matched rays carry their cosine similarity
    n_iterations     : int
...
def main():
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #
    # Pool only matched-view rays (reference rays excluded).  RANSAC finds
    # the 3-D point closest to the largest weighted subset of rays, then
    # refines with a weighted least-squares fit over all inliers.
    # ──────────────────────────────────────────────────────────────────────────
    ...
    print(f"  Rays in pool: {n_match_rays} matched-view rays  "
    ...
    # (Legend removed for a cleaner 3D view)
    ...
    plt_3d.show(*actors, interactive=False)
    # Zoom the camera in so the 3D object is more prominent
    plt_3d.camera.Zoom(1.8)
    plt_3d.interactive()
