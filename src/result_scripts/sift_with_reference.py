...
def ransac_ray_intersection(
    ...
    Hypothesis generation always draws one ray from the reference tube and
    one from the matched-view pool.  Scoring uses the weighted inlier sum.
    ...
    weights          : (N,)    — reference rays carry REFERENCE_RAY_WEIGHT,
                                 matched rays carry their similarity weight
...
def main():
    ...
    # ── Build unified ray arrays for RANSAC (ref + matched) ───────────────────
    rans_origins_list    = []
    rans_directions_list = []
    rans_weights_list    = []
    rans_ref_flags       = []

    # Reference ray — always included with REFERENCE_RAY_WEIGHT
    rans_origins_list.append(ref_ray["origin"])
    rans_directions_list.append(ref_ray["direction"])
    rans_weights_list.append(REFERENCE_RAY_WEIGHT)
    rans_ref_flags.append(True)
    ...
    for vn in matched_views:
        ...
        rans_ref_flags.append(False)

    if len(rans_origins_list) < 2:
        print("  [ERROR] Not enough rays for RANSAC "
    ...
    rans_ref_mask = np.array(rans_ref_flags,       dtype=bool)

    n_ref_rays   = int(rans_ref_mask.sum())
    n_match_rays = int((~rans_ref_mask).sum())

    print(f"\n  Rays in pool: {len(rans_origins)} total  "
          f"({n_ref_rays} reference @ w={REFERENCE_RAY_WEIGHT:.1f}  |  "
          f"{n_match_rays} matched @ w=1/dist)")
    ...
    best_pt, inlier_mask, residual = ransac_ray_intersection(
        rans_origins, rans_dirs, rans_weights, rans_ref_mask,
